...
class PixArtMSBlock(nn.Module):
    ...
    def forward(self, x, y, t, mask=None, HW=None, **kwargs):
        ...
        out, attn = self.cross_attn(x, y, mask)
        x = x + out
...
class PixArtMS(PixArt):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, return_attn=False, **kwargs):
        """
        ...
        for block in self.blocks:
            x, attn = auto_grad_checkpoint(block, x, y, t0, y_lens, (self.h, self.w), **kwargs)  # (N, T, D) #support grad checkpoint
            attn_list.append(attn)
        attn = torch.mean(torch.stack(attn_list), dim=0)
    ...
    def forward_with_dpmsolver(self, x, timestep, y, data_info, **kwargs):
        ...
        model_out, attn = self.forward(x, timestep, y, data_info=data_info, return_attn=True, **kwargs)
        return model_out.chunk(2, dim=1)[0], attn
