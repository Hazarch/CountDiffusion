...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        if mask is not None:
            ...
            attn_bias = torch.zeros(B, k.shape[2], N, k.shape[1]).to(device=q.device, dtype=q.dtype)
            for j, mk in enumerate(mask):
                attn_bias[j,...,mk:] = -1e3
        # x = xformers.ops.memory_efficient_attention(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)

        # attn.shape == [B, num_heads, head_dim, num_token]
        x, attn = self.memory_efficient_attention_pytorch(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)
        x = x.reshape(B, -1, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn
    
    def memory_efficient_attention_pytorch(self, query, key, value, attn_bias=None, p=0., scale=None):
